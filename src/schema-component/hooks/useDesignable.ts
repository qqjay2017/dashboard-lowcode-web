import type { GeneralField, Query } from '@formily/core'
import type {
  ISchema,
} from '@formily/react'
import {
...
function generateUid(s: ISchema) {
  if (!s['x-uid']) {
    s['x-uid'] = uid()
  }
...
function matchSchema(source: ISchema, target: ISchema) {
  if (!source || !target) {
    return
  }
...
export function splitWrapSchema(wrapped: Schema, schema: ISchema) {
...
export class Designable {
  ...
  insertBeforeBegin(schema: ISchema, options: InsertAdjacentOptions = {}) {
    ...
    const opts: any = { onSuccess: options.onSuccess }
    const {
      wrap = defaultWrap,
      breakRemoveOn,
