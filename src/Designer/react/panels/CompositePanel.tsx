...
function parseItems(children: React.ReactNode): React.PropsWithChildren<ICompositePanelItemProps>[] {
  const items = []
  React.Children.forEach(children, (child: any, index) => {
    if (child?.type === CompositePanelItem) {
...
function findItem(items: React.PropsWithChildren<ICompositePanelItemProps>[], key: string | number) {
