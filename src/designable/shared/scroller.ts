...
export function calcAutoScrollBasicInfo(point: IPoint, axis: 'x' | 'y', viewport: DOMRect, maxSpeed = MAX_SPEED): IAutoScrollBasicInfo | null {
...
export function updateScrollValue(element: HTMLElement | Window, axis: 'x' | 'y', value: number, callback?: (scrollValue: number) => void) {
  if (element) {
    if (!isWindow(element)) {
      if (axis === 'x') {
...
export function scrollAnimate(element: HTMLElement | Window, axis: 'x' | 'y', direction: 'begin' | 'end', speed: number, callback?: (scrollValue: number) => void) {
